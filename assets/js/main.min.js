        ...
        if (_item == _req) {
            if (_clicked == 0) {
                ...
                if ($phone != '' && $phone !== undefined) {
                    ...
                    if($phone.match(letters) || $phone.length < 9)
                    {
                        _clicked = 0;
                        Swal.fire({
                            icon: 'error',
...
function handleChangeInput(e) {
    ...
    if (_data[_p.attr('id')] != '') {
        ...
        if(_p.attr('id') === 'position' || _p.attr('id') === 'location') {
...
function sendForm(_data) {
    ...
    $('.btn-send').addClass('btn-send--sending');
 
    if (_data['fullname'] != '' && _data['fullname'] !== undefined) {
        user_name = _data['fullname']
    }
    if (_data['phone'] != '' && _data['phone'] !== undefined) {
        user_phone = _data['phone']
    }
    if (_data['email'] != '' && _data['email'] !== undefined) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result == 'success') {
                ...
                $('.btn-send').removeClass('btn-send--sending');
                trackingFormBanner(user_name, user_phone, user_email);
                window.dataLayer = window.dataLayer || [];
                dataLayer.push({
                    "event": "voteEvent",
                    "vote_category": "survey_msb_survey1_20220917", // survey Unicef , Survey PMI ,Survey Ban IV
                    "vote_action": "survey_msb_survey1_20220917" //Survey PMI 
                })
                Swal.fire({
                    ...
                    text: 'Cảm ơn bạn đã tham gia khảo sát của chúng tôi.',
...
function trackingFormBanner(user_name, user_phone, user_email) {
    ...
    logForm.push({
        "campaign_name": "survey_msb_survey1_20220917", //name của ca
        "engagement_type": "1003",
        "source_name": "Vnexpress", // Hệ thống tự push vào
        "source_type": "internal",
        "index_brand": "survey_msb_survey1_20220917",
        "index_industrial": "",
        "index_category": "",
        "index_topic": "",
